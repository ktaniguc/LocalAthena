...
if [ -f "$INPUT_LIST" ] ; then
  ...
  do
    ...
    #COMMAND="bsub -q 2d -o log.out 'source $TestArea/../build/$CMTCONFIG/setup.sh && Reco_tf.py  '--inputRDO_FTKFile=${INPUT_FILE}' '--maxEvents=500' '--postExec' 'all:CfgMgr.MessageSvc().setError+=["HepMcParticleLink"]' 'ESDtoAOD:fixedAttrib=[s if "CONTAINER_SPLITLEVEL = \'99\'" not in s else "" for s in svcMgr.AthenaPoolCnvSvc.PoolAttributes];svcMgr.AthenaPoolCnvSvc.PoolAttributes=fixedAttrib' 'RAWtoESD:topSequence.TrigSteer_HLT.enableRobRequestPreparation=False' '--postInclude' 'default:PyJobTransforms/UseFrontier.py' '--preExec' 'all:rec.Commissioning.set_Value_and_Lock(True);from AthenaCommon.BeamFlags import jobproperties;jobproperties.Beam.numberOfCollisions.set_Value_and_Lock(20.0);from LArROD.LArRODFlags import larRODFlags;larRODFlags.NumberOfCollisions.set_Value_and_Lock(20);larRODFlags.nSamples.set_Value_and_Lock(4);larRODFlags.doOFCPileupOptimization.set_Value_and_Lock(True);larRODFlags.firstSample.set_Value_and_Lock(0);larRODFlags.useHighestGainAutoCorr.set_Value_and_Lock(True)' 'all:from InDetRecExample.InDetJobProperties import InDetFlags; InDetFlags.doSlimming.set_Value_and_Lock(False)' 'all:from TriggerJobOpts.TriggerFlags import TriggerFlags;TriggerFlags.AODEDMSet.set_Value_and_Lock("AODFULL");' 'all:from JetRec.JetRecFlags import jetFlags,JetContentDetail; jetFlags.detailLevel=JetContentDetail.Full' 'RAWtoESD:rec.doFTK.set_Value_and_Lock(True)' '--skipEvents=0' '--autoConfiguration=everything' '--valid=True' '--conditionsTag' 'default:OFLCOND-MC16-SDR-17' '--geometryVersion=default:ATLAS-R2-2016-01-00-01' '--runNumber=361107' '--digiSeedOffset1=1911' '--digiSeedOffset2=1911' '--digiSteeringConf=StandardSignalOnlyTruth' '--AMITag=r10681' '--outputAODFile=${OUTPUT_AOD}' '--jobNumber=1911' '--triggerConfig=RAWtoESD=MCRECO:DBF:TRIGGERDBMC:2209,71,253' '--postInclude ./outputLevelConfig.py'"
